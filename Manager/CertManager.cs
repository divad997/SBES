    ...
    public class CertManager
    {
        ...
        public static X509Certificate2 GetCertificateFromStorage(StoreName storeName, StoreLocation storeLocation, string subjectName)
        {
            ...
            foreach (X509Certificate2 c in certCollection)
            {
                ...
                else if (c.SubjectName.Name.Equals(string.Format("CN={0}, OU=VIP", subjectName)))
                {
                    return c;
                }
                else if (c.SubjectName.Name.Equals(string.Format("CN={0}, OU=ADM", subjectName)))
                {
                    return c;
                }
                else if (c.SubjectName.Name.Equals(string.Format("CN={0}, OU=KOR", subjectName)))
                {
                    return c;
                }
